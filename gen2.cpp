...
int main(){
    ...
    while (op_realized < n){
        ...
        if(action == NOWY_PRZEDMIOT && objects_created < N_OBJECTS){
            cout << "NOWY_PRZEDMIOT\n";
        ...
        else if(action == NOWY_WOREK && objects_created < N_OBJECTS){
            cout << "NOWY_WOREK\n";
        ...
        else if(action == WLOZ_PRZEDMIOT && nr_w > 1 && nr_p > 1){
            int w = rand() % nr_w;
            int p = rand() % nr_p;
            if(przedmioty[p]->parent == &biurko && worki[w]->parent == &biurko){
                cout << "WLOZ_PRZEDMIOT " << p << ' ' << w << '\n';
                wloz(przedmioty[p], worki[w]);
                op_realized++;
            }
        }
        else if(action == WLOZ_WOREK && nr_w > 1){
            int w = rand() % nr_w;
            int w1 = rand() % nr_w;
            while(w == w1){
                w1 = rand() % nr_w;
            }
            if(worki[w1]->parent == &biurko && worki[w]->parent == &biurko){
                cout << "WLOZ_WOREK " << w1 << ' ' << w << '\n';
                wloz(worki[w1], worki[w]);
                op_realized++;
            }
        }
